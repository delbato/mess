...
use crate::codegen::output::Output as OutputVM;
...
impl Core {
    ...
    #[inline]
    pub fn load_program(&mut self, program: OutputVM) {
    ...
    pub fn get_opcode(&mut self) -> CoreResult<Opcode> {
        ...
        let opcode = Opcode::from_u8(op).ok_or(CoreError::InvalidOpcode(op))?;
    ...
    pub fn run_fn(&mut self, uid: u64) -> CoreResult<()> {
        let fn_offset = {
            let program = self.program.as_ref().ok_or(CoreError::NoProgram)?;
            program
                .functions
                .get(&uid)
            ...
            match opcode {
                ...
                Opcode::HALT => {
                    ...
                    match err_code {
                        ...
                        _ => return Err(CoreError::Halted(err_code)),
                    };
                }
                Opcode::MOVB => {
                    let lhs: u8 = self.get_op()?;
                    let rhs: u8 = self.get_op()?;
                    let boolean: bool = { self.reg(lhs)?.get() };
                    self.reg(rhs)?.set(boolean);
                }
                Opcode::MOVF => {
                    let lhs: u8 = self.get_op()?;
                    let rhs: u8 = self.get_op()?;
                    let float: f32 = { self.reg(lhs)?.get() };
                    self.reg(rhs)?.set(float);
                }
                Opcode::MOVI => {
                    let lhs: u8 = self.get_op()?;
                    let rhs: u8 = self.get_op()?;
                    let int64: i64 = { self.reg(lhs)?.get() };
                    self.reg(rhs)?.set(int64);
                }
                Opcode::MOVA => {
                    let lhs: u8 = self.get_op()?;
                    let rhs: u8 = self.get_op()?;
                    let uint64: u64 = { self.reg(lhs)?.get() };
                ...
                Opcode::MOVB_A => {
                    ...
                    let lhs_addr: u64 = { self.reg(lhs_reg)?.get() };
                    let rhs_addr: u64 = { self.reg(rhs_reg)?.get() };
                ...
                Opcode::MOVB_RA => {
                    ...
                    let rhs_offset: i16 = self.get_op()?;
                    let rhs_addr: u64 = { self.reg(rhs_reg)?.get() };
                    let boolean: bool = { self.reg(lhs_reg)?.get() };
                ...
                Opcode::MOVF_RA => {
                    ...
                    let rhs_offset: i16 = self.get_op()?;
                    let rhs_addr: u64 = { self.reg(rhs_reg)?.get() };
                    let float: f32 = { self.reg(lhs_reg)?.get() };
                ...
                Opcode::MOVI_RA => {
                    ...
                    let rhs_offset: i16 = self.get_op()?;
                    let rhs_addr: u64 = { self.reg(rhs_reg)?.get() };
                    let int64: i64 = { self.reg(lhs_reg)?.get() };
                ...
                Opcode::MOVA_RA => {
                    ...
                    let rhs_offset: i16 = self.get_op()?;
                    let rhs_addr: u64 = { self.reg(rhs_reg)?.get() };
                    let uint64: u64 = { self.reg(lhs_reg)?.get() };
                ...
                Opcode::ADDI => {
                    ...
                    let lhs: i64 = { self.reg(lhs_reg)?.get() };
                    let rhs: i64 = { self.reg(rhs_reg)?.get() };
                ...
                Opcode::ADDU => {
                    ...
                    let rhs_reg: u8 = self.get_op()?;
                    let target_reg: u8 = self.get_op()?;
                    let lhs: u64 = { self.reg(lhs_reg)?.get() };
                    let rhs: u64 = { self.reg(rhs_reg)?.get() };
                    self.reg(target_reg)?.set(lhs + rhs);
                }
                Opcode::SUBU => {
                    let lhs_reg: u8 = self.get_op()?;
                ...
                Opcode::ADDF => {
                    ...
                    let rhs_reg: u8 = self.get_op()?;
                    let target_reg: u8 = self.get_op()?;
                    let lhs: f32 = { self.reg(lhs_reg)?.get() };
                    let rhs: f32 = { self.reg(rhs_reg)?.get() };
                    self.reg(target_reg)?.set(lhs + rhs);
                }
                Opcode::SUBF => {
                    let lhs_reg: u8 = self.get_op()?;
                ...
                Opcode::JMPT => {
                    let lhs_reg: u8 = self.get_op()?;
                    let target_ip: u64 = self.get_op()?;
                    let lhs: bool = { self.reg(lhs_reg)?.get() };
                    if lhs {
                        self.ip.set(target_ip);
                    }
                }
                ...
                Opcode::DJMP => {
                    let lhs_reg: u8 = self.get_op()?;
                    let target_ip: u64 = { self.reg(lhs_reg)?.get() };
                    self.ip.set(target_ip);
                }
                Opcode::DJMPT => {
                    let lhs_reg: u8 = self.get_op()?;
                    let rhs_reg: u8 = self.get_op()?;
                    let target_ip: u64 = { self.reg(rhs_reg)?.get() };
                    let lhs: bool = { self.reg(lhs_reg)?.get() };
                    if lhs {
                        self.ip.set(target_ip);
                    }
                ...
                Opcode::AND => {
                    ...
                    let target_reg: u8 = self.get_op()?;
                    let lhs: bool = { self.reg(lhs_reg)?.get() };
                    let rhs: bool = { self.reg(rhs_reg)?.get() };
                    self.reg(target_reg)?.set(lhs && rhs);
                }
                Opcode::OR => {
                    let lhs_reg: u8 = self.get_op()?;
    ...
    fn mem_mov_n(&mut self, lhs: (u64, i16), rhs: (u64, i16), n: usize) -> CoreResult<()> {
        ...
        let bytes = {
            let source: &[u8] = match lhs_addr.address_type {
                AddressType::Stack => &self.stack,
                AddressType::Program => {
                    let program = self.program.as_ref().ok_or(CoreError::Unknown)?;
                    &program.code
                }
                AddressType::Swap => &self.swap,
        ...
        match rhs_addr.address_type {
            ...
            AddressType::Program => {
                let program = self.program.as_mut().ok_or(CoreError::Unknown)?;
                for i in 0..n {
    ...
    pub fn mem_get_string(&self, addr: u64) -> CoreResult<String> {
        ...
        String::from_utf8(string_data).map_err(|_| CoreError::OperatorDeserialize)
    }
    ...
    pub fn mem_get<T: DeserializeOwned>(&self, addr: (u64, i16)) -> CoreResult<T> {
        ...
        deserialize(&data).map_err(|_| CoreError::OperatorDeserialize)
    }
    ...
    pub fn mem_set<T: Serialize>(&mut self, addr: (u64, i16), item: T) -> CoreResult<()> {
        ...
        let data = serialize(&item).map_err(|_| CoreError::OperatorSerialize)?;
    ...
    pub fn get_foreign_ptr<T>(&self, ptr: u64) -> CoreResult<Arc<Mutex<T>>> {
        let arc_box_int = self.foreign_pointers.get(&ptr).ok_or(CoreError::Unknown)?;
        let arc = unsafe {
    ...
    fn get_op<T: DeserializeOwned + Debug>(&mut self) -> CoreResult<T> {
        ...
        let ret: T = deserialize(raw_bytes).map_err(|_| CoreError::OperatorDeserialize)?;
    ...
    pub fn push_stack<T: Serialize>(&mut self, item: T) -> CoreResult<()> {
        ...
        let raw_bytes = serialize(&item).map_err(|_| CoreError::OperatorSerialize)?;
    ...
    pub fn pop_stack<T: DeserializeOwned>(&mut self) -> CoreResult<T> {
        ...
        deserialize(&raw_bytes).map_err(|_| CoreError::Unknown)
    }
